...
if (port === 443) {
    ...
    redirectApp.use('/.well-known', express.static(path.join(__dirname, './front/.well-known')));
    redirectApp.get('/', function (req, res) {
        res.set('Content-Type', 'text/html');
        res.send(new Buffer('<html><head><meta http-equiv="refresh" content="0; url=' + settings.url + '" /></head></html>'));
    });
